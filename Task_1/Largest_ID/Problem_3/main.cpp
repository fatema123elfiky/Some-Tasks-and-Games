...
/**
 * @brief Reads and processes words from a file.txt stream.
 *
 * @param in The input file.txt stream to read data from.
 * @return vector<string> A vector containing all processed words from the file.txt.
 *
 ...
 * to select whether to display the map in the console or write it to a file.txt.
 */
void printWordFrequency(const vector<string>& words) {
    ...
    string Menu = "Do you want input to be in a file.txt or console?\n1) Console\n2) File\nYour Choice: ";
    ...
    if (choice == "1") {
        ...
        if(frequencyMap.empty()){
            cout << "No words found in the input file.\n";
            return;
        }
        ...
        while (true) {
            ...
            if (!file.is_open())
                cout << "Error opening the file.txt. Please try again.\n";
            else
                break;
        }
        if(frequencyMap.empty()){
            file << "No words found in the input file.\n";
            return;
        }
        for (const auto& pair : frequencyMap)
...
int main() {
    ...
    while (true) {
        // Prompt for file.txt name and validate its existence.
        while (true) {
            cout << "Enter the name of the file.txt: ";
            getline(cin, filename);
            in.open(filename);
        ...
        // Process file.txt content and calculate word frequencies.
...
>>>>>>> 1edee9c977057e167a4f4ba18ee0b89858fa575e
